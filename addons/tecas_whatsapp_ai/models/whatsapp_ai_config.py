...
class TecasWhatsappAIConfig(models.Model):
    _name = 'tecas.whatsapp.ai.config'
    _description = 'TECAS WhatsApp AI Bot Configuration'
    ...
    def get_singleton(self):
        ...
        if not config:
            ...
            raw_ids = ICP.get_param('tecas_whatsapp_ai.salesman_ids', '')
            last_idx = int(ICP.get_param('tecas_whatsapp_ai.last_salesman_index', '-1') or '-1')
            user_ids = [int(x) for x in raw_ids.split(',') if x.strip().isdigit()]
            config = self.sudo().create({
                'salesman_ids': [(6, 0, user_ids)] if user_ids else [],
                'last_salesman_index': last_idx,
            })
        return config
    ...
    def action_open(self):
        ...
        return {
            ...
            'name': 'Bot',
            'res_model': 'tecas.whatsapp.ai.config',
            'view_mode': 'form',
            'res_id': config.id,
            'target': 'current',
